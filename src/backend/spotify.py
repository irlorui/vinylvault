...
@contextmanager
def _spotify_op():
    """Wrap a Spotify call, converting Spotify errors to HTTPException."""
    try:
        yield
...
def fetch_all_tracks_enriched(sp: spotipy.Spotify, playlist_id: str) -> list[dict]:
    """Fetch every track from a playlist with extended metadata for ETL.
...
def get_random_track(
    ...
    if exclude is not None:
        available = [t for t in available if t["id"] not in exclude]
    if not available:
